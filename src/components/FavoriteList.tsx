...
type Props = {
    ...
    handleConfirmFromFavorite: (url: string) => void;
};

export const FavoriteList = ({ favoriteList, deleteFavorite, handleConfirmFromFavorite }: Props) => {
    return (
        <>
            ...
            {favoriteList.filter(video => video.favorite).map((video) => (
                ...
                >
                    <div className="flex justify-between items-center gap-3 w-full rounded bg-slate-800 px-4 py-4">
                        <span 
                            className="flex cursor-pointer items-center rounded hover:text-blue-500"
                            onClick={() => handleConfirmFromFavorite(video.url)}
                        >{video.title}</span>
